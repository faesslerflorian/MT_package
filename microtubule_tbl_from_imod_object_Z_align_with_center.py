...
#Aplly scale_contour to contours in an model
def scale_model(model, scale):
    output_model = []
    for contour in model:
        new_contour = scale_contour(contour, scale)
        output_model.append(new_contour)
    return output_model
...
def sample_tube_at_one_position(point, direction_vector, tube_radius, sampling_rate_circle):
    ...
    eulers_current_vector = [tdrot_current_vector, tilt_current_vector, 0]
    coordinates_and_direction_vectors_of_all_sampling_points = [[[round(point[0],2),
                                                                  round(point[1],2),
                                                                  round(point[2],2)],
